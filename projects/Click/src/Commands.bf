...
{
	public String ParseError = new .() ~ delete _;
	...
	public Result<void> Dispatch(StringView cmd, Span<StringView> _args)
	{
		if (!ContainsKey(cmd))
		{
			//Errors.Report($"Unknown subcommand {cmd}");
			ParseError..Clear().AppendF($"Unknown subcommand {cmd}");
			return .Err;
		}
			...
			else if (field.FieldType == typeof(String))
			{
				...
				for (let arg in argIter)
				{
					...
					if (arg.StartsWith("--") && flagName == argName)
					{
						...
						{
							ParseError..Clear().AppendF($"Missing value after {arg}");
							return .Err;
						}
					}
					else if (attr.Short != null && arg.StartsWith("-") && flagName.Length == 1 && flagName[0] == attr.Short)
				...
				{
					ParseError..Clear().AppendF($"Argument '{argName}' is required");
					return .Err;
				}
			}
		}
